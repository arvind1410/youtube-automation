...
def _search_and_dl_yt_video(
    ...
    if len(search.results) == 0:
        logger(r"No video found by query: {search_query}")
        return None
    ...
    for v in search.results:
        try:
            ...
            logger.error(r"Error occurred at {v.title}: {e}")
            continue
        if stream:
...
def get_clips(
    ...
    with VideoFileClip(path) as main_clip:
        if n_clips * clips_duration > main_clip.duration:
            logger.error(
                r"80% of main_clip duration {main_clip.duration} isn't enough for {n_clips} clips by {clips_duration} seconds"
            )
            return
        ...
        for i, t_start in enumerate(subclip_start_times):
            f_name = r"{output_folder}/videos/{n_paragraph}_{i}.mp4"
            with main_clip.subclip(t_start, t_start + clips_duration) as new_clip:
                ...
                logger.info(r"FILE SAVED: {f_name}")
    try:
        shutil.rmtree(r"{output_folder}/videos/yt")
    except Exception:
        pass
...
def get_random_subclip_start_times(
    ...
    for _ in range(number_of_tries):
        ...
        if all([tup[i] + clips_duration <= tup[i + 1] for i in range(n_clips - 1)]):
            logger.info(r"Work with timings: {tup}")
            return tup
    else:
        logger.error(
            r"Somehow couldn't find suitable subclip timings in {number_of_tries} tries..."
        )
        return []
